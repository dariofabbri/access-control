...
public class QueryAccessoByStatoPostazionePassiDestinatarioAutorizzatoDaIngressoDaIngressoAUscitaDaUscitaANomeVisitatoreCognomeVisitatore extends Query<Accesso> {
	...
	public QueryAccessoByStatoPostazionePassiDestinatarioAutorizzatoDaIngressoDaIngressoAUscitaDaUscitaANomeVisitatoreCognomeVisitatore(Session session) {
	...
	protected String getCountHql() {
		...
		String hql = 
				...
				"left join acc.postazione pos " +
				"where 1 = 1 ";

		if(idStato != null)
			hql += "and sta.id = :idStato ";

		if(idPostazione != null)
			hql += "and pos.id = :idPostazione ";
		
		if(passi != null)
			hql += "and upper(acc.passi) like :passi ";
	...
	protected void setQueryArguments(org.hibernate.Query q) {
		...
		for (int i = 0; i < named_params.length; ++i) {
			...
			else if (param.equals("idPostazione"))
				q.setParameter("idPostazione", idPostazione);

			else if (param.equals("passi"))
				q.setParameter("passi", "%" + passi.toUpperCase() + "%");
